...
class PolicyNetwork(object):
	...
	def __init__(self, scope = 'policy_network', learning_rate = 0.001):
		...
		with tf.compat.v1.variable_scope(scope):
			...
			self.loss = tf.reduce_sum(-tf.compat.v1.log(self.picked_action_prob)*self.target) + sum(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES, scope=scope))
